...
{     
	...
	public function slider_entry(Request $r)
	{
		...
		return redirect()->action('Ecm\SliderImageController@index')->with('success','Data saved successfuly');  		 	
	}
	...
	{	
	  ...
	  //       ->editColumn('slider_title', function ($result) {
			...
			->editColumn('action', function ($result) {
			  return "<a class='btn btn-xs btn-success' href='". url('/admin/slider-image-edit/'. $result->id .'') ."'>Edit</a>&nbsp;".
			   			    "<a class='btn btn-xs btn-danger' href='". url('/admin/slider-image-delete/'. $result->id .'') ."'>Delete</a>";
			})	
			->make(true);
	}
	...
	public function slider_edit_action(Request $r)
	{
		...
		return redirect('/admin/slider-image-list')->with('success','Slider updated successfuly');  
	}
