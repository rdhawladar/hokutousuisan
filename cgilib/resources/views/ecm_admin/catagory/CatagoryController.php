...
{     
	...
	public function catagory_entry(Request $r)
	{
		...
		return redirect()->action('Ecm\CatagoryController@index')->with('success','Data saved successfuly');  		 	
	}
	...
	{	
	  ...
	  //       ->editColumn('catagory_title', function ($result) {
			...
			->editColumn('action', function ($result) {
			  return "<a class='btn btn-xs btn-success' href='". url('/admin/catagory-edit/'. $result->id .'') ."'>Edit</a>&nbsp;".
			   			    "<a class='btn btn-xs btn-danger' href='". url('/admin/catagory-delete/'. $result->id .'') ."'>Delete</a>";
			})	
			->make(true);
	}
	...
	public function catagory_edit_action(Request $r)
	{
		...
		return redirect('/admin/catagory-list')->with('success','Data updated successfuly');  
	}
